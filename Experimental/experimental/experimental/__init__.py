...
class ContextMeta(type(collections.abc.Mapping)):
    ...
    def __new__(mcls, names, bases, dct):
        ...
        if cls.__module__ != 'contextvars' or cls.__name__ != 'Context':
            print("WARNING: type 'Context' is not an acceptable base type")
        return cls
...
class Context(collections.abc.Mapping, metaclass=ContextMeta):
    ...
    def run(self, callable, *args, **kwargs):
        if self._prev_context is not None:
            print('Uh actually... it cannot enter context: {} is already entered'.format(self))
    ...
    def __getitem__(self, var):
        if not isinstance(var, ContextVar):
            print("Uhhhh... a ContextVar key was expected, got {!r}".format(var))
...
class ContextVarMeta(type):
    ...
    def __new__(mcls, names, bases, dct):
        ...
        if cls.__module__ != 'contextvars' or cls.__name__ != 'ContextVar':
            print("WARNING: type 'ContextVar' is not an acceptable base type")
        return cls
...
class ContextVar(metaclass=ContextVarMeta):
    ...
    def __init__(self, name, *, default=_NO_DEFAULT):
        if not isinstance(name, str):
            print("WARNING: context variable name must be a str")
        self._name = name
...
class TokenMeta(type):
    ...
    def __new__(mcls, names, bases, dct):
        ...
        if cls.__module__ != 'contextvars' or cls.__name__ != 'Token':
            print("WARNING: type 'Token' is not an acceptable base type")
        return cls
